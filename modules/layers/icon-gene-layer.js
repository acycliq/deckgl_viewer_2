// IconGeneLayer: IconLayer with GPU gene visibility mask (1D texture LUT)
// - Based on ScatterplotGeneLayer but for custom glyph rendering
// - Adds per-instance geneId attribute (binary path)
// - Multiplies fragment alpha by visibility[geneId] sampled from a 1D texture
// - Supports binary data format for high performance with millions of spots

const {IconLayer} = deck;

export class IconGeneLayer extends IconLayer {
  initializeState() {
  ...
  getShaders() {
    ...
    // Inject attribute, varying, and sampler for gene visibility masking
    const inject = {
  ...
  _updateMaskTexture(maskArray) {
    ...
    gl.bindTexture(gl.TEXTURE_2D, tex);
    gl.pixelStorei(gl.UNPACK_ALIGNMENT, 1);
    // Build LUMINANCE texture of size (width x 1), store mask in luminance channel
    try {
      gl.texImage2D(
        gl.TEXTURE_2D,
        0,
      ...
      // Fallback to RGBA packing (should not be necessary but just in case)
